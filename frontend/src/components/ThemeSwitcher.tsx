...
export default function ThemeSwithcer() {
    ...
    return (
        <div className="fab fab-flower right-5 bottom-10">
            ...
            {themeData.map((item) => (
                <div className="tooltip tooltip-left text-base-content" data-tip={item.title} key={item.name} onClick={() => setTheme(item.name)}>
                    <button className={`btn btn-circle btn-lg btn-soft ${item.btn}`}>{item.icon}</button>
                </div>
            ))}
        </div>
    )
